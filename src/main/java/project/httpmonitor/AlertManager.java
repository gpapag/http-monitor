package project.httpmonitor;

import project.httpmonitor.pojo.LogRecord;
...
{
    private static final String ALERT_MSG = "High traffic generated an alert - hits = %d, triggered at %d";
    ...
    {
        while (!eventQueue.isEmpty() && eventQueue.peekFirst().minusSeconds(alertIntervalSec).isAfter(eventQueue.peekLast())) {
    ...
    {
        String alertMessage = String.format(ALERT_MSG, eventQueue.size(), eventQueue.size() != 0 ? eventQueue.peekLast().getEpochSecond() : 0);
    ...
    {
        String clearAlertMessage = String.format(CLEAR_ALERT_MSG, eventQueue.size() != 0 ? eventQueue.peekLast().getEpochSecond() : 0);
